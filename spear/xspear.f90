...
INTEGER i,j
REAL*8 d,probd,probrs,rs,zd
REAL*8 data1(NDAT),data2(NDAT),rays(NDAT,NMON)
REAL*8 wksp1(NDAT),wksp2(NDAT),ave(NDAT),zlat(NDAT)
CHARACTER city(NDAT)*15,mon(NMON)*4,text*64
open(7,file='/Users/dingding 1/Desktop/spear/TABLE2.DAT',status='OLD')
read(7,*)
read(7,'(a)') text
